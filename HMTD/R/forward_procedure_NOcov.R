#'Estimates the Forward probabilities (without visible level covariates)
#'
#'@param Data is a single univariate data sequence
#'@return 1. scaled forward probabilities, 2. scaling constants and 3.Incomplete data log-Likelihood
...
forward_procedure_NOcov<-function(Data, n,r,k,OHC,kOHC,Pi,A,Phi,P,Theta,Q,spec,...){
    ...
    for(j in 1:k){                                      ### calculer mu et sigma de la seq. "Data", utilisant Phi et Theta de chaque composante.
        ...
        output=max(1/(sqrt(2*pi)*sigma)*exp(-(Data[t]-mu)^2/(2*sigma^2)),1e-100)  # pr chaq compos calculer la pdf de la loi normale (avec son mu et sigma) de la donn?e actuelle # parfois =0 car valeurs < min de la machine
