SEM_v2_componentwise<- function(fct, VECTPAR,lengthPhi,lengthTheta,k, target=NA, it, envir, lower= VECT_limits_min ,upper= VECT_limits_max,mutheta,sigtheta,muphi,sigphi,initialVECTPAR=NULL){
    ...
    changes<-min_param_change#array(0.2, c(1,lengthVECTPAR))
    ...
    while(nbr_fcalls<it){
        ...
        for (m in seq(component, length(VECTPAR), k)){
            ...
            if(LL_up>LL_last){
                ...
                if(direction[m]=="up"){changes[m]=min(changes[m]*2,max_param_change[m])}else{changes[m]=max(changes[m]/3,min_param_change[m])} # if the last direction las the same increase the step, if not diminish it because we may have gone too far.
                direction[m]="up"
            } else{
                ...
                if(LL_down>LL_last){
                    ...
                    if(direction[m]!="up"){changes[m]=min(changes[m]*2,max_param_change[m])}else{changes[m]=max(changes[m]/3,min_param_change[m])} # if the last direction was the same increase the step, if not diminish it because we may have gone too far.
                    direction[m]="down"
                } else{
