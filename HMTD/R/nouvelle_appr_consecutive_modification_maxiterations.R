...
nouvelle_appr_consecutive_modification_maxiterations<- function(fct, VECTPAR, lengthPhi, lengthTheta, target, maxdelta, mindelta, it, envir...){
    ...
    while(nbr_fcalls<it){
        ...
        for (m in 1:(lengthVECTPAR)){
            ...
            VECTPAR_down[m]=VECTPAR[m]-max(VECTPAR[m]*changes[m],0.005)
        ...
        if(Lvisible[ii]>max(Lvisible[Lvisible<(-0.5)])){VECTPAR_elite=rbind(VECTPAR_elite,cbind(VECTPAR,ii))  # if last logL is the best so far, save VECTPAR !
        #VECTPAR=VECTPAR+rnorm(length(VECTPAR))*0.2*VECTPAR
        #LL_last=fct(VECTPAR)
        #jj=jj+1
        }
        if(Lvisible[ii]==Lvisible[ii-1]){VECTPAR_elite=rbind(VECTPAR_elite,cbind(t(VECTPAR),ii))  # if last logL is the best so far, save VECTPAR !
        VECTPAR=VECTPAR+rnorm(length(VECTPAR))*0.2*VECTPAR
        LL_last=fct(VECTPAR)
        jj=jj+1
        }
    ...
    #return=list(max(Lvisible[Lvisible<(-0.5)]), VECTPAR_elite[which(VECTPAR_elite[,length(VECTPAR)+1]==  which(Lvisible==max(Lvisible[Lvisible<(-0.5)])) ), 1:length(VECTPAR) ], jj, nbr_fcalls)
