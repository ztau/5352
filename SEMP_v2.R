    ...
    while(nbr_fcalls<it){
        ...
        for (m in importance_order){  ### We update the parameters in the order of their last absolute logL improvement
            ...
            if(LL_up>LL_last){
                ...
                if(direction[m]=="up"){changes[m]=min(changes[m]*2,max_param_change)}else{changes[m]=max(changes[m]/1.5,min_param_change)} # if the last direction las the same increase the step, if not diminish it because we may have gone too far.
                direction[m]="up"
            } else{
                ...
                if(LL_down>LL_last){
                    ...
                    if(direction[m]!="up"){changes[m]=min(changes[m]*2,max_param_change)}else{changes[m]=max(changes[m]/1.5,min_param_change)} # if the last direction was the same increase the step, if not diminish it because we may have gone too far.
                    direction[m]="down"
                } else{
