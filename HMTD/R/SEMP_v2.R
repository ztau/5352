#'Maximization procedure "SEMP_v2"
#'
...
SEMP_v2<- function(fct, VECTPAR,lengthPhi,lengthTheta,k, target=NA, it, envir, lower= VECT_limits_min ,upper= VECT_limits_max){
    ...
    min_param_change=amplitude/200 # the minimum and the maximum of the step are calculated as fractions of the amplitude between the limits (parameter space)
    ...
    while(nbr_fcalls<it){
        ...
        for (m in importance_order){  ### We update the parameters in the order of their last absolute logL improvement
            ...
            jj=jj+1
            if(LL_up>LL_last){
                LL_delta[m]=LL_up-LL_last
                LL_last=LL_up
                VECTPAR[m]=VECTPAR_up[m]
                if(direction[m]=="up"){changes[m]=min(changes[m]*2,max_param_change[m])}else{changes[m]=max(changes[m]/1.5,min_param_change[m])} # if the last direction las the same increase the step, if not diminish it because we may have gone too far.
                direction[m]="up"
            } else{
                LL_down=fct(VECTPAR_down)       # logL values obtaned if we reduce the parameters (for each parameter separately)
                jj=jj+1
                if(LL_down>LL_last){
                    LL_delta[m]=LL_down-LL_last
                    LL_last=LL_down
                    VECTPAR[m]=VECTPAR_down[m]
                    changes[m]=min(changes[m]*2,max_param_change[m])
                    if(direction[m]!="up"){changes[m]=min(changes[m]*2,max_param_change[m])}else{changes[m]=max(changes[m]/1.5,min_param_change[m])} # if the last direction was the same increase the step, if not diminish it because we may have gone too far.
                    direction[m]="down"
                } else{
        ...
        ii=ii+1
        nbr_fcalls=jj
        if ((ii/10)%%1==0){    # save every 10th parameter proposition (just for information how did we explore the parameter space)
