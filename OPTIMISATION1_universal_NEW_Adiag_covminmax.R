OPTIMISATION1_universal_NEW_Adiag_covminmax<-function(Data,  X_cov, Phi_cov, phi_cov_places, k,OHC, P,Q,procedure=c("SEMP"), it, envir, Adiag, covmin=NA, covmax=NA,...){
    ...
    # Le cas de A diagonale ou non-diagonale est traité par "LL_after_hidden_level_reestimation_..."
    
    library("nnet")
    
    source("hmtd_mu_cov.R")       # pour utiliser la fonction actualisée après un changement
    ...
    if(length(X_cov)<2){
        ...
        # mettre tous les parametres dans un sel vecteur
        VECTPAR=c(Theta, Phi, Phi_cov[Phi_cov!=0])   
        lengthVECTPAR=length(VECTPAR)
